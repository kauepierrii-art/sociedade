...
(function () {
  ...
  function renderProcedureDocument() {
    ...
    const archive = researchReferences.map(reference => {
      return '<article class="reference-file dossier-record" data-reference="' + reference.id + '">' +
        '<button class="dossier-toggle" type="button" aria-expanded="false"><span>' + reference.code + ' — ' + reference.title + '</span><span class="chev">＋</span></button>' +
        '<div class="dossier-content" hidden><header><span>' + reference.code + '</span><h3>' + reference.title + '</h3><small>REGISTRO PRESERVADO</small></header>' +
        ...
        '<button class="research-button" type="button" data-research="' + reference.id + '">APROFUNDAR PESQUISA</button></div></article>';
    }).join('');
    actions.innerHTML = '<section class="recovered-dossier">' +
      ...
      '<article class="dossier-document dossier-record"><button class="dossier-toggle" type="button" aria-expanded="false"><span>DOCUMENTO I — NOTA DE PROCEDIMENTO</span><span class="chev">＋</span></button><div class="dossier-content" hidden><p class="dossier-label">DOCUMENTO I</p><h2>NOTA DE PROCEDIMENTO</h2><div class="dossier-copy">' + paragraphs(procedureText) + '</div></div></article>' +
      '<div class="dossier-divider"></div>' +
      '<article class="dossier-document dossier-record"><button class="dossier-toggle" type="button" aria-expanded="false"><span>DOCUMENTO II — REFERÊNCIAS PRESERVADAS</span><span class="chev">＋</span></button><div class="dossier-content" hidden><p class="dossier-label">DOCUMENTO II</p><h2>REFERÊNCIAS PRESERVADAS</h2><div class="dossier-copy">' + paragraphs(referenceText) + '</div></div></article>' +
    ...
    actions.querySelectorAll('.dossier-toggle').forEach(toggle => {
