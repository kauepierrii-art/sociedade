...
(function () {
  ...
  function enableProtocolClue() {
    ...
    quote.innerHTML = '<a href="#arquivo-etapa-06">“Nem todos os olhos veem o mesmo.”</a>';
  ...
  function renderInvestigationArchive() {
    ...
    const analysis = [
      ...
      'A passagem menciona <strong>doze sinais dispostos ao redor da superfície</strong>.',
      'Apenas <strong>cinco deveriam permanecer</strong>.',
      ...
      '<strong>Cinco indivíduos parecem ocorrer de maneira indireta e recorrente no conjunto.</strong>',
      '<strong>Identifique-os.</strong>'
    ];
    const reconstruction = [
      ...
      'Determine as datas associadas às cinco referências.'
    ];
    const annexes = [
    ...
    const annexMarkup = annexes.map(item =>
      '<article class="archive-annex"><button type="button" aria-expanded="false"><span>ANEXO ' + item[0] + ' — ' + item[1] + '</span><span class="archive-chev">＋</span></button><div hidden></div></article>'
    ).join('');

    actions.innerHTML =
      '<section class="stage6-archive">' +
        '<header class="archive-header"><p>ARQUIVO RECUPERADO</p><small>CLASSIFICAÇÃO: PROCEDIMENTO / SUPERFÍCIE REFLEXIVA</small><h2>NEM TODOS OS OLHOS VEEM O MESMO.</h2></header>' +
        '<article class="archive-document"><p class="archive-label">MATERIAL HISTÓRICO VERIFICÁVEL</p><h3>CONTEXTO DOCUMENTAL</h3><div class="archive-copy">' + paragraphs(historical) + '</div>' +
          '<p class="archive-sources">Referências: <a href="https://www.bl.uk/manuscripts/FullDisplay.aspx?ref=Sloane_MS_3188" target="_blank" rel="noopener">British Library</a> · <a href="https://hsm.ox.ac.uk/holy-table" target="_blank" rel="noopener">History of Science Museum</a> · <a href="https://www.britishmuseum.org/collection/object/H_1906-0922-1" target="_blank" rel="noopener">British Museum</a></p></article>' +
        '<div class="archive-divider"></div>' +
        '<article class="archive-document archive-analysis"><p class="archive-label">PROCEDÊNCIA NÃO CONFIRMADA</p><h3>MATERIAL EM ANÁLISE</h3><div class="archive-copy">' + paragraphs(analysis) + '</div></article>' +
        '<div class="archive-divider"></div>' +
        '<section class="archive-annexes"><h3>ANEXOS PRESERVADOS</h3>' + annexMarkup + '</section>' +
        '<div class="archive-divider"></div>' +
        '<article class="archive-document"><h3>RECONSTRUÇÃO</h3><div class="archive-copy">' + paragraphs(reconstruction) + '</div></article>' +
    ...
    actions.querySelectorAll('.archive-annex button').forEach(button => button.addEventListener('click', () => {
      const open = button.getAttribute('aria-expanded') === 'true';
      button.setAttribute('aria-expanded', String(!open));
      button.querySelector('.archive-chev').textContent = open ? '＋' : '−';
  ...
  style.textContent = `
    ...
    @media(max-width:520px){.zodiac-mark{transform:rotate(calc(var(--mark) * 30deg)) translateY(calc(-1 * min(43vw, 145px))) rotate(calc(var(--mark) * -30deg));font-size:24px}.mirror-v2-field{width:100%;max-width:340px}.stage6-archive{font-size:15px}.archive-analysis{padding-left:12px}.correspondence-form{gap:9px}.correspondence-form button{width:100%}}
  `;
