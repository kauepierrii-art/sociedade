...
(function () {
  ...
  const correctRunes = ['rune_04', 'rune_11', 'rune_03', 'rune_07', 'rune_14', 'rune_18', 'rune_21'];
  const runeSymbols = ['✢','⌬','⟡','✦','⊹','⟁','☽','⟟','✧','⨳','♜','◈','✣','⨯','☉','⟠','✥','⚚','⟢','⩘','✺'];
  ...
  function renderMirrorStage() {
    ...
    if (!Array.isArray(remaining) || remaining.length !== 7 && remaining.length !== 21 && !remaining.every(id => /^rune_\d\d$/.test(id))) remaining = remainDefault();
    ...
    actions.innerHTML = '<section class="mirror-procedure ' + (unlocked ? 'is-unlocked' : 'is-sealed') + (activated ? ' is-activated' : '') + '">' +
      ...
      '</div><p class="mirror-status" role="status"></p>' +
