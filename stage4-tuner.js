...
(function () {
  ...
  function renderTuner() {
    ...
    panel.innerHTML = `
      ...
      <button class="stage4-replay-video" type="button" hidden>REASSISTIR MENSAGEM</button>`;
    ...
    const replayButton = panel.querySelector('.stage4-replay-video');
    ...
    replayButton.addEventListener('click', () => showSupportVideo());
    ...
    if (reviewMode) {
      ...
      values.forEach((_, index) => {
        updateKnob(index);
        const knob = knobs.children[index];
        if (knob) knob.disabled = true;
      });
      on = true;
      panel.classList.add('is-on', 'is-stabilized');
      lever.disabled = true;
      lever.setAttribute('aria-pressed', 'true');
      lever.setAttribute('aria-label', 'Alavanca travada — sinal estabilizado');
      led.setAttribute('aria-label', 'Sinal estabilizado');
      status.textContent = 'FREQUÊNCIA CONFIRMADA — SINAL ESTABILIZADO.';
  ...
  reviewStyle.textContent = '.stage4-tuner.is-stabilized{box-shadow:inset 0 0 0 2px #0b0e0b,inset 0 0 25px rgba(87,214,178,.18),0 8px 18px rgba(0,0,0,.24)}.stage4-tuner.is-stabilized .stage4-led{background:#75dfbd;box-shadow:0 0 9px rgba(117,223,189,.85)}.stage4-knob:disabled{opacity:1;cursor:default}.stage4-replay-video{display:block;margin:14px auto 1px;padding:8px 12px;border:1px solid rgba(87,214,178,.7);background:rgba(7,30,25,.68);color:var(--green);font:700 9px "IBM Plex Mono",monospace;letter-spacing:.11em;cursor:pointer}.stage4-replay-video:hover{background:rgba(87,214,178,.12);color:#c7f1e4}';
